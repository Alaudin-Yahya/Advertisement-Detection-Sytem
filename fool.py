...
def Graph(list):
    ...
    u = 0
    root = tk.Tk()
    canvas = tk.Canvas(root, width=800, height=1000, borderwidth=0, highlightthickness=0, bg="white")
    canvas.grid()
    tk.Canvas.create_circle_arc = _create_circle_arc
    start = 90;
    canvas.create_circle(600, 200, 150, fill="blue")
    colors = ["violet","indigo","green","red","orange","aqua"]
    j=0
    l=0
    # w = Canvas(root, width=250, height=200)
    while (u < len(list)):
        dur = int((list[u + 1] - list[u])/2);
        d = dur*per
        # print("haad",dur)
        u = u + 2;
        end = start+d
        canvas.create_circle_arc(600, 200, 150, fill=colors[j], outline="", start=start, end=end)
        start+=d
        canvas.create_rectangle(450, 400 + l, 465, 415 + l, fill=colors[j], outline='')
        aaa = os.path.basename(AdvertismentFiles[j])
        aaa = aaa.replace(".mp4","")
        label = tk.Label(canvas, text=aaa, font=("Calibri", 13))
        label.pack()
        label.place(x=470, y=393 + l)
        l = l + 20
        j+=1
    i = 0;
    j = 0;
    canvas.create_line(0, 650, TotalFramesOfMainVideo / 2, 650, fill="black", width=10);
    while (i < len(list)):
        start = list[i];
        end = list[i + 1]
        canvas.create_line(start / 2, 650, end / 2, 650, fill=colors[j], width=12)
        i = i + 2;
        j = j + 1
